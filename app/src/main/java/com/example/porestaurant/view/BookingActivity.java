...
public class BookingActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        tvNoTables = findViewById(R.id.tv_no_tables);
    ...
    private void setupCapacitySpinner() {
        ...
        ArrayAdapter<String> capacityAdapter = new ArrayAdapter<>(this,
        ...
        call.enqueue(new Callback<List<TableDTO>>() {
            ...
            public void onResponse(Call<List<TableDTO>> call, Response<List<TableDTO>> response) {
                if (response.isSuccessful()) {
                    ...
                    Toast.makeText(BookingActivity.this, "Lỗi khi tải bàn trống: " + response.message(), Toast.LENGTH_SHORT).show();
                }
            }

            @Override
            public void onFailure(Call<List<TableDTO>> call, Throwable t) {
                Toast.makeText(BookingActivity.this, "Kết nối thất bại: " + t.getMessage(), Toast.LENGTH_SHORT).show();
            }
        });
    }
            ...
            public void onResponse(Call<List<TableDTO>> call, Response<List<TableDTO>> response) {
                if (response.isSuccessful() && response.body() != null && position >= 0 && position < response.body().size()) {
                    ...
                    if (capacity > table.getCapacity()) {
                        Toast.makeText(BookingActivity.this, "Số người vượt quá sức chứa bàn!", Toast.LENGTH_SHORT).show();
                        return;
                    }

                    // Kiểm tra thời gian không phải quá khứ
                    if (calendar.getTime().compareTo(new Date()) < 0) {
                        Toast.makeText(BookingActivity.this, "Thời gian đặt không hợp lệ", Toast.LENGTH_SHORT).show();
                        return;
                    }
                    ...
                    bookCall.enqueue(new Callback<TableDTO>() {
                        ...
                        public void onResponse(Call<TableDTO> call, Response<TableDTO> response) {
                            if (response.isSuccessful()) {
                                Toast.makeText(BookingActivity.this, "Đặt bàn thành công!", Toast.LENGTH_SHORT).show();
                                loadAvailableTables();
                            } else {
                                Log.e("Booking", "Failed: " + response.code() + " - " + response.message());
                                Toast.makeText(BookingActivity.this, "Đặt bàn thất bại: " + response.message(), Toast.LENGTH_SHORT).show();
                            }
                        }
                    ...
                    Toast.makeText(BookingActivity.this, "Vui lòng chọn bàn hợp lệ", Toast.LENGTH_SHORT).show();
                }
            }
